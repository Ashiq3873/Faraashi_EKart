...
def send_email(to_email, subject, html_body, text_body=None, attachment_data=None, attachment_filename=None):
    if not to_email:
        return False
    try:
        msg = MIMEMultipart('mixed')
        msg['Subject'] = subject
        msg['From'] = f'Faraashi-Ekart Admin <{ADMIN_EMAIL}>'
        msg['To'] = to_email
        msg['Reply-To'] = ADMIN_EMAIL
        
        alt_part = MIMEMultipart('alternative')
        alt_part.attach(MIMEText(text_body or html_body, 'plain'))
...
def build_admin_email(title, body_html, accent='#6366f1'):
    return f"""
    <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto; color: #0f172a;">
        <div style="background: linear-gradient(135deg, #0f172a, {accent}); padding: 28px 24px; border-radius: 12px 12px 0 0; text-align: center;">
            <p style="margin: 0 0 8px; color: rgba(255,255,255,0.75); font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase;">Faraashi-Ekart Official</p>
            <h1 style="color: #fff; margin: 0; font-size: 22px;">{title}</h1>
        </div>
        <div style="background: #ffffff; border: 1px solid #e2e8f0; border-top: none; padding: 28px 24px; border-radius: 0 0 12px 12px; line-height: 1.7;">
            {body_html}
            <hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
            <p style="margin: 0; color: #64748b; font-size: 14px;">
                This message was sent from <strong>{ADMIN_EMAIL}</strong><br>
                Faraashi-Ekart Admin · Karaikudi, Tamil Nadu
            </p>
        </div>
    </div>
    """

def get_product_media(product):
    if not product:
        return [{'type': 'image', 'url': 'https://via.placeholder.com/600x600?text=No+Image'}]
    media = product.get('media')
    if isinstance(media, list) and media:
        return sorted(media, key=lambda x: 0 if x.get('type') == 'video' else 1)
    if product.get('image_url'):
        return [{'type': 'image', 'url': product['image_url']}]
    return [{'type': 'image', 'url': 'https://via.placeholder.com/600x600?text=No+Image'}]

def get_product_cover_image(product):
    for item in get_product_media(product):
        if item.get('type') == 'image':
            return item.get('url')
    media = get_product_media(product)
    return media[0].get('url') if media else 'https://via.placeholder.com/400x400?text=No+Image'

def upload_media_files(file_list):
    uploaded = []
    for media_file in file_list:
        if not media_file or not media_file.filename:
            continue
        try:
            result = cloudinary.uploader.upload(media_file, resource_type='auto')
            media_type = 'video' if result.get('resource_type') == 'video' else 'image'
            uploaded.append({'type': media_type, 'url': result['secure_url']})
        except Exception as e:
            print(f'[Cloudinary Upload Error] {e}')
    return uploaded

@app.template_filter('product_media')
def product_media_filter(product):
    return get_product_media(product)

@app.template_filter('product_cover')
def product_cover_filter(product):
    return get_product_cover_image(product)
...
def build_review_reply_email(review, ai_reply):
    ...
    html_body = build_admin_email(
        'Thank You for Your Review',
        f"""
        <p>Dear <strong>{user_name}</strong>,</p>
        <p>Thank you for reviewing <strong>{target_name}</strong>. Your feedback helps us improve every day.</p>
        <div style="background:#f8fafc; padding:16px; border-left:4px solid #64748b; margin: 20px 0; border-radius:4px;">
            <p style="margin:0 0 8px 0; font-size:14px; color:#64748b; text-transform:uppercase; font-weight:bold;">Your Review</p>
            <p style="margin:0; font-style:italic;">"{review_text}"</p>
        </div>
        <div style="background:#f0fdf4; padding:16px; border-left:4px solid #10b981; margin: 20px 0; border-radius:4px;">
            <p style="margin:0 0 8px 0; font-size:14px; color:#047857; text-transform:uppercase; font-weight:bold;">Admin Response</p>
            <p style="margin:0;">{ai_reply}</p>
        </div>
        <p>Warm regards,<br><strong>Faraashi-Ekart Admin Team</strong></p>
        """,
        accent='#059669',
    )
    text_body = (
        f"Dear {user_name},\n\n"
        f"Thank you for submitting a review for {target_name}. We truly appreciate your feedback and your time!\n\n"
        f"Your Review:\n\"{review_text}\"\n\n"
        f"Admin Reply:\n{ai_reply}\n\n"
        f"Thank you again for shopping with Faraashi Ekart. If you have any further questions, feel free to contact us.\n\n"
        f"Warm regards,\nThe Faraashi Ekart Team\n"
    )
...
def index():
    ...
    screened_reviews = list(db.reviews.find({"screen": True}).sort("created_at", -1)) if db is not None else []
    
    return render_template('index.html', products=products, blogs=blogs, categories=categories, screened_reviews=screened_reviews)
...
def blog_detail(blog_id):
    if not session.get('user') and not session.get('is_admin'):
        flash('Please login to view this blog.', 'info')
...
def place_order():
    ...
    # Use registered email for sending the confirmation
    registered_email = session['user'].get('email') or form_email
    ...
    if db is not None:
        try:
            ...
            order_data = db.orders.find_one(get_id_query(order_id))
            if order_data and registered_email:
                pdf_buffer = generate_invoice_pdf_buffer(order_data)
                ...
                for item in cart_items:
                    ...
                    items_html += (
                        f"<tr>"
                        f"<td style='padding:10px; border-bottom:1px solid #e2e8f0;'>{item.get('name', 'Product')}</td>"
                        f"<td style='padding:10px; border-bottom:1px solid #e2e8f0; text-align:center;'>{item.get('qty', 1)}</td>"
                        f"<td style='padding:10px; border-bottom:1px solid #e2e8f0; text-align:right;'>₹{line_total:.2f}</td>"
                        f"</tr>"
                    )
                
                est_delivery = datetime.fromtimestamp(datetime.now().timestamp() + 5*24*3600).strftime("%d %b %Y")
                
                html_body = build_admin_email(
                    f"Order Confirmation #{order_id.split('.')[0]}",
                    f"""
                    <p>Dear <strong>{first_name} {last_name}</strong>,</p>
                    <p>Thank you for shopping with Faraashi-Ekart. Your order has been confirmed and is being processed.</p>
                    <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                        ...
                        <tfoot>
                            <tr>
                                <td colspan="2" style="text-align: right; padding: 12px 10px; font-weight: bold;">Order Total</td>
                                <td style="padding: 12px 10px; font-weight: bold; text-align:right; color:#6366f1;">₹{total:.2f}</td>
                            </tr>
                        </tfoot>
                    </table>
                    <div style="background-color: #f8fafc; padding: 16px; border-radius: 8px; margin-bottom: 16px;">
                        <h4 style="margin: 0 0 8px; color: #0f172a;">Shipping Address</h4>
                        <p style="margin: 0;">{address}, {city} - {pin_code}<br>Phone: {phone}</p>
                    </div>
                    <p><strong>Estimated Delivery:</strong> {est_delivery}</p>
                    <p>Your invoice is attached as a PDF. Track your order anytime from your account.</p>
                    <p>Best regards,<br><strong>Faraashi-Ekart Admin Team</strong></p>
                    """,
                )
                text_body = (
                    f"Dear {first_name} {last_name},\n\n"
                    f"Your order #{order_id.split('.')[0]} is confirmed. Total: ₹{total:.2f}\n"
                    f"Delivery address: {address}, {city} - {pin_code}\n\n"
                    f"Faraashi-Ekart Admin Team\n{ADMIN_EMAIL}"
                )
                send_email(
                    registered_email,
                    f"Order Confirmation #{order_id.split('.')[0]} — Faraashi-Ekart",
                    html_body,
                    text_body,
                    pdf_buffer.getvalue(),
                    f"invoice_{order_id.split('.')[0]}.pdf"
                )
...
def support():
    ...
    return render_template('support.html', tab=tab, orders=orders, blogs=blogs)
...
def login():
    ...
    if request.method == 'POST':
        ...
        if username == 'admin' and password == 'Faraashi013':
...
def register():
    ...
    if request.method == 'POST':
        ...
        if db is not None:
            ...
            # Send welcome acknowledgement from admin
            shop_url = url_for('products', _external=True)
            welcome_html = build_admin_email(
                'Welcome to Faraashi-Ekart',
                f"""
                <p>Dear <strong>{username}</strong>,</p>
                <p>Thank you for registering with Faraashi-Ekart. Your account has been created successfully.</p>
                <p>You can now shop premium products, track orders, download invoices, and share reviews — all from your account dashboard.</p>
                <div style="text-align: center; margin: 28px 0;">
                    <a href="{shop_url}" style="background-color: #6366f1; color: #ffffff; padding: 12px 24px; text-decoration: none; border-radius: 8px; font-weight: bold;">Start Shopping</a>
                </div>
                <p>We are glad to have you with us. For any help, reply to this email anytime.</p>
                <p>Best regards,<br><strong>Faraashi-Ekart Admin Team</strong></p>
                """,
            )
            welcome_text = (
                f"Dear {username},\n\n"
                f"Thank you for registering with Faraashi-Ekart. Your account is ready.\n"
                f"Shop now: {shop_url}\n\n"
                f"Faraashi-Ekart Admin Team\n{ADMIN_EMAIL}"
            )
            if send_email(email, 'Welcome to Faraashi-Ekart — Registration Confirmed', welcome_html, welcome_text):
                flash('Registration successful! A welcome email has been sent to your inbox.', 'success')
            else:
                flash('Registration successful! Please login. (Welcome email could not be sent.)', 'success')
...
@app.route('/admin/update_stock/<pid>', methods=['POST'])
def update_stock(pid):
    ...
    try:
        ...
        if add_stock > 0:
            db.products.update_one(get_id_query(pid), {"$inc": {"stock": add_stock}})
            flash('Stock updated successfully!', 'success')
        else:
            flash('Please enter a valid stock quantity to add.', 'danger')
    except Exception as e:
        flash('Invalid stock value.', 'danger')
    return redirect(url_for('admin_dashboard'))

@app.route('/admin/delete_review/<rid>')
def delete_review(rid):
    if not session.get('is_admin') or db is None:
        return redirect(url_for('login'))
    db.reviews.delete_one(get_id_query(rid))
    flash('Review deleted successfully!', 'success')
    return redirect(url_for('admin_dashboard') + '#customer-reviews')
...
def admin_add_product_page():
    ...
    return render_template('admin/add_product.html', now=datetime.now())

@app.route('/admin/blogs/add')
def admin_add_blog_page():
    if not session.get('is_admin'): return redirect(url_for('login'))
    return render_template('admin/add_blog.html', now=datetime.now())

@app.route('/admin/inventory')
def admin_inventory():
    if not session.get('is_admin'): return redirect(url_for('login'))
    products = list(db.products.find()) if db is not None else []
    return render_template('admin/inventory.html', products=products, now=datetime.now())

@app.route('/admin/orders')
def admin_orders():
    if not session.get('is_admin'): return redirect(url_for('login'))
    orders = list(db.orders.find().sort("created_at", -1)) if db is not None else []
    return render_template('admin/orders.html', orders=orders, now=datetime.now())

@app.route('/admin/reviews')
def admin_reviews():
    if not session.get('is_admin'): return redirect(url_for('login'))
    reviews = list(db.reviews.find().sort("created_at", -1)) if db is not None else []
    return render_template('admin/reviews.html', reviews=reviews, now=datetime.now())

@app.route('/admin/review_screen/<review_id>/<action>')
def admin_review_screen(review_id, action):
    if not session.get('is_admin') or db is None:
        return redirect(url_for('login'))
    is_screened = (action == 'on')
    db.reviews.update_one(get_id_query(review_id), {"$set": {"screen": is_screened}})
    flash(f"Review screen turned {'on' if is_screened else 'off'}.", "success")
    return redirect(url_for('admin_reviews'))

@app.route('/admin/blogs_archive')
def admin_blogs_archive():
    if not session.get('is_admin'): return redirect(url_for('login'))
    blogs = list(db.blogs.find().sort("created_at", -1)) if db is not None else []
    return render_template('admin/blogs_archive.html', blogs=blogs, now=datetime.now())
...
def add_product():
    ...
    if files:
        uploaded_media = upload_media_files(files)
        if uploaded_media:
            media_list.extend(uploaded_media)

    image_url = media_list[0]['url'] if media_list else "https://via.placeholder.com/400"
